...
@torch.no_grad()
def smooth_ln_fcs_bert(fcs, act_scales, alpha=0.5):
    if not isinstance(fcs, list):
        fcs = [fcs]
    for fc in fcs:
        assert isinstance(fc, nn.Linear)
...
@torch.no_grad()
def smooth_lm(model, scales, alpha=0.5, hidden_size = 768):
    for name, module in model.named_modules():
        if isinstance(module, BertLayer):
            qkv = [
                module.attention.self.Wqkv[:hidden_size],
                module.attention.self.Wqkv[hidden_size:2 * hidden_size],
                module.attention.self.Wqkv[2 * hidden_size:] ,
            ]
            qkv_input_scales = scales[name + ".attention.self.q_proj"]
            smooth_ln_fcs_bert(qkv, qkv_input_scales, alpha)

            fc1 = module.mlp
            fc1_input_scales = scales[name + ".mlp"]
            smooth_ln_fcs_bert(fc1, fc1_input_scales, alpha)
