...
def quantize_dnabert(
    ...
    for name, m in model.bert.named_modules():
        if type(m).__name__ == 'BertGatedLinearUnitMLP':
        # if isinstance(m, BertGatedLinearUnitMLP):
            m.gated_layers = W8A8Linear.from_float(
                m.gated_layers, weight_quant=weight_quant, act_quant=act_quant
            )

            m.wo = W8A8Linear.from_float(
                m.wo, weight_quant=weight_quant, act_quant=act_quant
            )
        elif type(m).__name__ == 'BertUnpadAttention':
        #elif isinstance(m, BertUnpadAttention):

            # Quantize the output dense layer
            m.output.dense = W8A8Linear.from_float(
                m.output.dense,
            ...
            # Quantize the Wqkv layer
            m.self.Wqkv = W8A8Linear.from_float(
                m.self.Wqkv,
                weight_quant=weight_quant,
                act_quant=act_quant,
                quantize_output=quantize_bmm_input,
