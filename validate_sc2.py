...
from transformers_language.models.quantized_bert import QuantizedBertForSequenceClassification
...
class ModelArguments:
    ...
    attn_softmax: str = field(
        default='vanilla',
        metadata={
            "help": (
                "softmax function defination"
            )
        },
    )
    ...
    quantize: bool = field(
        default=False,
        metadata={"help": "Will enable quantization."},
    )
    est_num_batches: int = field(
        default=1,
    )
    n_bits: int = field(
        default=8,
    )
    n_bits_act: int = field(
        default=8,
    )
    no_weight_quant: bool = field(
        default=False,
    )
    no_act_quant: bool = field(
        default=False,
    )
    ranges_weights : str = field(
        default='minmax',
    )
    ranges_acts : str = field(
        default='running_minmax',
    )
    qmethod_acts: str = field(
        default='asymmetric_uniform',
    )
    percentile: float = field(
        default=None,
    )
    quant_setup: str = field(
        default='all',
    )
...
def main():
    ...
    for layer_idx in range(len(model.bert.encoder.layer)):
        ...
        new_self = BertSelfAttentionWithExtras(
            ...
            softmax_fn=SOFTMAX_MAPPING[model_args.attn_softmax],
            ssm_eps=None,
            tau=None,
    ...
    if model_args.quantize:
        ...
        click_config.act_quant.num_batches = model_args.est_num_batches
        click_config.quant.n_bits = model_args.n_bits
        click_config.quant.n_bits_act = model_args.n_bits_act
        if model_args.no_weight_quant:
            click_config.quant.weight_quant = False
        if model_args.no_act_quant:
            click_config.quant.act_quant = False

        # Weight Ranges
        if model_args.ranges_weights == "minmax":
            pass
        elif model_args.ranges_weights in ("mse", "MSE"):
            click_config.quant.weight_quant_method = RangeEstimators.MSE
            click_config.quant.weight_opt_method = OptMethod.grid
        else:
            raise ValueError(f"Unknown weight range estimation: {model_args.ranges_weights}")

        # Acts ranges
        if model_args.percentile is not None:
            click_config.act_quant.options["percentile"] = model_args.percentile

        if model_args.ranges_acts == "running_minmax":
            click_config.act_quant.quant_method = RangeEstimators.running_minmax

        elif model_args.ranges_acts == "MSE":
            click_config.act_quant.quant_method = RangeEstimators.MSE
            if model_args.qmethod_acts == "symmetric_uniform":
                click_config.act_quant.options = dict(opt_method=OptMethod.grid)
            elif model_args.qmethod_acts == "asymmetric_uniform":
                click_config.act_quant.options = dict(opt_method=OptMethod.golden_section)

        elif model_args.ranges_acts.startswith("L"):
            click_config.act_quant.quant_method = RangeEstimators.Lp
            p_norm = float(model_args.ranges_acts.replace("L", ""))
            ...
            raise NotImplementedError(f"Unknown act range estimation setting, '{model_args.ranges_acts}'")
