...
def get_weight_modules(model, config_model):
    ...
    weight_modules = []
    if config_model.model_type == 'bert':
        for i in range(num_layer):
            layer = getattr(model, config_model.model_type).encoder.layer[i]
            # self attn, q, k, v
            weight_modules.append([
                layer.attention.self.Wqkv])
            # linear
            weight_modules.append([layer.mlp.gated_layers])
    elif config_model.model_type == 'bert2' or config_model.model_type == 'roberta':
        ...
        for i in range(num_layer):
            ...
            # linear
            weight_modules.append([layer.mlp])
        ...
        if isinstance(module, GammaResidual):
            if last_layernorm is not None:
                ...
                for w in weight_modules[cnt]:
                    ...
                    # print(old_module.layernorm.weight.data.shape)
