...
from quant_transformer.solver.utils import dnabert_utils
...
def main(config_path):
    config = dnabert_utils.parse_config(config_path)
    set_seed(config.train.seed)
    if config.data.task_name == 'dnabert':
    ...
    # raw_datasets = dnabert_utils.load_dataset_labels(config.data, config.model)
    config.model.max_seq_length =  config.data.max_seq_length
    tokenizer, model = dnabert_utils.load_model(config.model, config.data)

    # max_seq_length
    config.data.max_seq_length = min(config.data.max_seq_length, tokenizer.model_max_length)

    train_datasets = dnabert_utils.SupervisedDataset(tokenizer=tokenizer, 
                                      data_path=config.data.train_file, 
                                      kmer=config.data.kmer)
    eval_datasets = dnabert_utils.SupervisedDataset(tokenizer=tokenizer, 
                                     data_path=config.data.validation_file, 
                                     kmer=config.data.kmer)
    test_datasets = dnabert_utils.SupervisedDataset(tokenizer=tokenizer, 
                                     data_path=config.data.test_file, 
                                     kmer=config.data.kmer)
    data_collator = dnabert_utils.DataCollatorForSupervisedDataset(tokenizer=tokenizer)
