...
def safe_norm(x, p=np.inf):
    x = x.view(x.size(0), -1)
    return x.norm(dim=1, p=p)
...
class ModelArguments:
    ...
    low_cpu_mem_usage: bool = field(
        ...
        metadata={
            "help": (
                "It is an option to cr2eate the model as an empty shell, then only materialize its parameters when the pretrained weights are loaded. "
...
def main():
    ...
    with torch.no_grad():  # No gradient needed for evaluation
        for batch_idx, batch in enumerate(tqdm(eval_loader)):
            ...
            # Assuming model outputs the loss if labels are provided
    ...
    try:
        perplexity = math.exp(average_loss) if average_loss is not None else float('inf')
    ...
    metrics["eval_loss"] = average_loss
