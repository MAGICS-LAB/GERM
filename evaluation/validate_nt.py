...
def main():
    ...
    for layer_idx in range(len(model.esm.encoder.layer)):
        ...
        print("----------------------------------------------------------")
        print("Inside Esm custom attention")
        print("----------------------------------------------------------")
    ...
    if args.quantize:
        ...
        model = QuantizedEsmForSequenceClassification(model, **qparams)
    ...
    print(click_config.quant.n_bits)
    print(click_config.quant.n_bits_act)
    print(model.esm.encoder.layer[0].attention.self.query)
    
    print(model.esm.encoder.layer[0].attention.self.query.weight.dtype) #float32
    #exit(0)
    ...
    for batch_idx, batch in enumerate(tqdm(eval_dataloader)):
        ...
        if EXTRA_METRICS:
            for j in range(num_layers):
                for name in (
                    f"esm.encoder.layer.{j}.output.dense",  # FFN output
                    f"esm.encoder.layer.{j}.LayerNorm",  # LN(FFN output + input)
                ):
    ...
    if EXTRA_METRICS:
        ...
        max_ffn_out_inf_norm = max(v.avg for k, v in act_inf_norms.items() if "output.dense" in k)
