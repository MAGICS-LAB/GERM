...
class QuantizedEsmContactPredictionHead(QuantizedModel):
    """Performs symmetrization, apc, and computes a logistic regression on the output features"""

    def __init__(
        self,
        org_module, **quant_params,
    ):
        self.quant_dict = _make_quant_dict(quant_params["quant_dict"])
        super().__init__()
        quant_params_ = quant_params.copy()
        ...
        self.regression = quantize_model(org_module.regression, **quant_params_)
        self.activation = quantize_model(org_module.activation, **quant_params)
...
class QuantizedEsmEmbeddings(QuantizedModel):
    """
    ...
    def __init__(self, 
        ...
        **quant_params,
        ...
        self.position_embeddings = quantize_model(org_module.position_embeddings, **quant_params)
...
class QuantizedEsmSelfAttention(QuantizedModel):
    def __init__(self, 
        org_module,
        ...
        **quant_params
        ...
        self.query = quantize_model(org_module.query, **quant_params)
        self.key = quantize_model(org_module.key, **quant_params)
        self.value = quantize_model(org_module.value, **quant_params)
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            self.distance_embedding = quantize_model(org_module.distance_embedding, **quant_params)
...
class QuantizedEsmSelfOutput(QuantizedModel):
    def __init__(self, org_module, **quant_params):
        self.quant_dict = _make_quant_dict(quant_params["quant_dict"])
        super().__init__()
        self.dense = quantize_model(org_module.dense, **quant_params)
        self.dropout = org_module.dropout
...
class QuantizedEsmAttention(QuantizedModel):
    def __init__(self, org_module, **quant_params):
        self.quant_dict = _make_quant_dict(quant_params["quant_dict"])
        super().__init__()
        
        self.self = QuantizedEsmSelfAttention(org_module.self, **quant_params)
        self.output = QuantizedEsmSelfOutput(org_module.output, **quant_params)
        ...
        if self.quant_dict.layer_norm_res_self_output:
            self.LayerNorm = QuantLayerNorm(
                org_module=org_module.LayerNorm,
                input_quantizer=self.res_act_quantizer.activation_quantizer.quantizer,
                **quant_params,
            )
        else:
            self.LayerNorm = quantize_model(org_module.LayerNorm, **quant_params)
...
class QuantizedEsmIntermediate(QuantizedModel):
    def __init__(self, org_module, **quant_params):
        self.quant_dict = _make_quant_dict(quant_params["quant_dict"])
        super().__init__()
        
        self.dense = quantize_model(org_module.dense, **quant_params)
        self.activation_fn = quantize_model(org_module.activation_fn, **quant_params)
...
class QuantizedEsmOutput(QuantizedModel):
    def __init__(self, org_module, **quant_params):
        self.quant_dict = _make_quant_dict(quant_params["quant_dict"])
        super().__init__()
...
class QuantizedEsmLayer(QuantizedModel):
    def __init__(self, org_module, **quant_params):
        self.quant_dict = _make_quant_dict(quant_params["quant_dict"])
        super().__init__()
        ...
        self.seq_len_dim = 1
        self.attention = QuantizedEsmAttention(org_module.attention, **quant_params)
        self.is_decoder = org_module.is_decoder
        self.add_cross_attention = org_module.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise RuntimeError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = QuantizedEsmAttention(org_module.crossattention, **quant_params)
        self.intermediate = QuantizedEsmIntermediate(org_module.intermediate, **quant_params)
        self.output = QuantizedEsmOutput(org_module.output, **quant_params)
        self.LayerNorm = quantize_model(org_module.LayerNorm, **quant_params)
...
class QuantizedEsmEncoder(QuantizedModel):
    def __init__(self, org_module, **quant_params):
        self.quant_dict = _make_quant_dict(quant_params["quant_dict"])
        super().__init__()
        ...
        for i in range(self.config.num_hidden_layers):
            self.layer.append(QuantizedEsmLayer(org_module.layer[i], **quant_params))
        self.emb_layer_norm_after = quantize_model(org_module.emb_layer_norm_after, **quant_params)
...
class QuantizedEsmPooler(QuantizedModel):
    def __init__(self, org_module, **quant_params):
        self.quant_dict = _make_quant_dict(quant_params["quant_dict"])
        super().__init__()
...
class QuantizedEsmModel(QuantizedModel, ModuleUtilsMixin):
    """
    ...
    def __init__(self, org_module, add_pooling_layer=False, **quant_params):
        self.quant_dict = _make_quant_dict(quant_params["quant_dict"])
        super().__init__()
        ...
        self.embeddings = QuantizedEsmEmbeddings(org_module.embeddings, self.config, **quant_params)
        self.encoder = QuantizedEsmEncoder(org_module.encoder, **quant_params)

        self.pooler = QuantizedEsmPooler(org_module.pooler, **quant_params) if self.add_pooling_layer else None

        self.contact_head = QuantizedEsmContactPredictionHead(
            org_module.contact_head, **quant_params
        )
...
class QuantizedEsmForSequenceClassification(QuantizedModel):
    def __init__(self, org_module, **quant_params):
        self.quant_dict = _make_quant_dict(quant_params["quant_dict"])
        super().__init__()
        ...
        self.esm = QuantizedEsmModel(org_module=org_module.esm, **quant_params)
        self.classifier = QuantizedEsmClassificationHead(org_module=org_module.classifier, **quant_params)
...
class QuantizedEsmClassificationHead(QuantizedModel):
    ...
    def __init__(self, org_module, **quant_params):
        ...
        self.dense = quantize_model(org_module.dense, **quant_params)
        self.dropout = org_module.dropout
        self.out_proj = quantize_model(org_module.out_proj, **quant_params)
